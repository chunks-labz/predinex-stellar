...
if command -v stellar >/dev/null 2>&1; then
  ...
  # CI runners do not ship the Stellar CLI. Fall back to Cargo so the build
  # stays deterministic without requiring a separate tool installation.
  cargo build \
    --manifest-path "$manifest_path" \
    --locked \
    --release \
  ...
  built_wasm="target/wasm32v1-none/release/$crate_basename.wasm"
  [ -f "$built_wasm" ] || {
    echo "No WASM artifact found at $built_wasm after cargo build" >&2
    exit 2
  }

  wasm_path="$out_dir/$crate_basename.wasm"
  cp "$built_wasm" "$wasm_path"
fi
