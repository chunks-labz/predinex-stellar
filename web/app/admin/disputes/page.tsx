...
export default function AdminDisputes() {
  ...
  const [selectedPool, setSelectedPool] = useState<ProcessedMarket | null>(
    null,
  );
  ...
  async function loadPools() {
    ...
    try {
      ...
      const allMarkets: ProcessedMarket[] = sorobanPools.map((pool) => {
        ...
        const oddsA =
          totalVolume > 0 ? Math.round((pool.totalA / totalVolume) * 100) : 50;
        const oddsB =
          totalVolume > 0 ? Math.round((pool.totalB / totalVolume) * 100) : 50;
        const timeRemaining =
          pool.expiry > blockHeight ? pool.expiry - blockHeight : null;
      ...
      const filtered = allMarkets.filter(
        (p) => p.status === "frozen" || p.status === "disputed",
      );
      setPools(filtered);
    } catch (err) {
  ...
  const handleAction = async (
    action: "freeze" | "dispute" | "unfreeze",
    poolId: number,
  ) => {
    ...
    try {
      ...
      showSuccess(`Successfully executed ${action} on pool ${poolId}`);
      loadPools();
                    ...
                    {!selectedPool ? (
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="space-y-3">
                          <button
                            disabled={
                              isSubmitting || selectedPool.status === "frozen"
                            }
                            onClick={() =>
                              handleAction("freeze", selectedPool.poolId)
                            }
                          ...
                          <button
                            disabled={
                              isSubmitting || selectedPool.status === "disputed"
                            }
                            onClick={() =>
                              handleAction("dispute", selectedPool.poolId)
                            }
                          ...
                          <button
                            disabled={
                              isSubmitting ||
                              (selectedPool.status !== "frozen" &&
                                selectedPool.status !== "disputed")
                            }
                            onClick={() =>
                              handleAction("unfreeze", selectedPool.poolId)
                            }
                          ...
                          {isSubmitting && (
                            <p className="text-sm text-center text-primary animate-pulse mt-2">
                              {getStageLabel(stage)}
                            </p>
                          )}
                        </div>
