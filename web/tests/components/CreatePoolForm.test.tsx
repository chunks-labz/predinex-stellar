...
/** Fill the two default outcome fields. */
async function fillDefaultOutcomes(user: ReturnType<typeof userEvent.setup>) {
  await user.type(screen.getByLabelText(/outcome 1/i), 'Yes');
  await user.type(screen.getByLabelText(/outcome 2/i), 'No');
}
...
describe('CreatePoolForm (#677)', () => {
  ...
  it('renders all required fields including two default outcome inputs', () => {
    ...
    expect(screen.getByLabelText(/outcome 2/i)).toBeInTheDocument();
  ...
  it('rejects unsupported asset types on submit', async () => {
    ...
    await fillForm(user);
    await fillDefaultOutcomes(user);
  ...
  it('invokes createPool with outcomes array on submit', async () => {
  ...
  it('clears the form via the clear button', async () => {
    ...
    expect([null, '']).toContain((screen.getByLabelText(/deposit amount/i) as HTMLInputElement).value);
    expect([null, '']).toContain((screen.getByLabelText(/expiry/i) as HTMLInputElement).value);
    // Clearing resets back to two empty outcomes
    expect(screen.getByLabelText(/outcome 1/i)).toHaveValue('');
    expect(screen.getByLabelText(/outcome 2/i)).toHaveValue('');
  });
...
describe('CreatePoolForm multi-outcome (#831)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    localStorage.clear();
    setWalletState();
  ...
  it('renders an "Add outcome" button and starts with exactly 2 outcome inputs', () => {
    renderWithProviders(<CreatePoolForm />);
    expect(screen.getByLabelText(/outcome 1/i)).toBeInTheDocument();
    expect(screen.getByLabelText(/outcome 2/i)).toBeInTheDocument();
    expect(screen.queryByLabelText(/outcome 3/i)).not.toBeInTheDocument();
    expect(screen.getByRole('button', { name: /add outcome/i })).toBeInTheDocument();
  });

  it('adds a third outcome when "Add outcome" is clicked', async () => {
    const user = userEvent.setup();
    renderWithProviders(<CreatePoolForm />);

    await user.click(screen.getByRole('button', { name: /add outcome/i }));

    expect(screen.getByLabelText(/outcome 3/i)).toBeInTheDocument();
  });

  it('removes an outcome when its remove button is clicked', async () => {
    const user = userEvent.setup();
    renderWithProviders(<CreatePoolForm />);

    // Add a third outcome first so the remove button appears
    await user.click(screen.getByRole('button', { name: /add outcome/i }));
    expect(screen.getByLabelText(/outcome 3/i)).toBeInTheDocument();

    await user.click(screen.getByRole('button', { name: /remove outcome 3/i }));
    expect(screen.queryByLabelText(/outcome 3/i)).not.toBeInTheDocument();
  });

  it('does not show remove buttons when only 2 outcomes are present', () => {
    renderWithProviders(<CreatePoolForm />);
    expect(screen.queryByRole('button', { name: /remove outcome/i })).not.toBeInTheDocument();
  });

  it('hides the "Add outcome" button when 10 outcomes are present', async () => {
    const user = userEvent.setup();
    renderWithProviders(<CreatePoolForm />);

    // Add outcomes until we reach 10
    for (let i = 0; i < 8; i++) {
      await user.click(screen.getByRole('button', { name: /add outcome/i }));
    }

    expect(screen.getByLabelText(/outcome 10/i)).toBeInTheDocument();
    expect(screen.queryByRole('button', { name: /add outcome/i })).not.toBeInTheDocument();
  });

  it('submits all 3 outcomes to createPool when a third is added', async () => {
    vi.mocked(Contract.createPool).mockResolvedValue({
      txHash: 'multi-outcome-tx',
    ...
    // Fill the two default outcomes
    await user.type(screen.getByLabelText(/outcome 1/i), 'Option A');
    await user.type(screen.getByLabelText(/outcome 2/i), 'Option B');

    // Add and fill a third
    await user.click(screen.getByRole('button', { name: /add outcome/i }));
    await user.type(screen.getByLabelText(/outcome 3/i), 'Option C');
    ...
    const [params] = vi.mocked(Contract.createPool).mock.calls[0];
    expect(params.outcomes).toEqual(['Option A', 'Option B', 'Option C']);
  });

  it('renders initialValues outcomes when supplied', () => {
    renderWithProviders(
      <CreatePoolForm initialValues={{ outcomes: ['Bull', 'Bear', 'Sideways'] }} />
    );
    expect(screen.getByLabelText(/outcome 1/i)).toHaveValue('Bull');
    expect(screen.getByLabelText(/outcome 2/i)).toHaveValue('Bear');
    expect(screen.getByLabelText(/outcome 3/i)).toHaveValue('Sideways');
  });
});
